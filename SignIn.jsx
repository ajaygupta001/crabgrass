...
function SignIn() {
  ...
  const submitHandler = async () => {
   ...
   // let result = await fetch(`https://crabgrassbackend.onrender.com/signin`, {
    ...
    if (result.sucess) {
      localStorage.setItem("CrabgrassSignin", JSON.stringify(result.data));
      navigate("/");
      setShowLog(true);
      setLoader(false);
    } else {
    ...
    <div className="bg-gradient-to-tl from-green-400 to-indigo-900 w-full h-screen px-4">
      <div className="flex flex-col items-center justify-center p-2">
        ...
        <img
          ...
          alt="logo"
        />
        <div className="bg-white shadow rounded lg:w-1/3  md:w-1/2 w-full p-10 mt-10 relative">
          <p
            tabIndex={0}
            aria-label="Login to your account"
            className="text-2xl font-extrabold leading-4 text-gray-800"
          >
          ...
          <div>
            {loader && (
              <div className="absolute top-30 ml-40">
                <Loading />
              </div>
            )}
